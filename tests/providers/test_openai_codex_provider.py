...
@pytest.mark.asyncio
async def test_codex_oauth_error_is_actionable_and_not_fallbackable(monkeypatch) -> None:
    def fake_token():
        raise RuntimeError(
            'Token refresh failed: 401 {"error":{"code":"refresh_token_reused"}}'
        )
    ...
    async def fake_request(*args: Any, **kwargs: Any):
        raise AssertionError("Codex request should not run when OAuth token loading fails")
    ...
    assert "Error calling Codex (CodexOAuthError)" in response.content
    assert "nanobot provider logout openai-codex" in response.content
    assert "nanobot provider login openai-codex" in response.content
    assert "refresh_token_reused" in response.content


@pytest.mark.asyncio
async def test_codex_oauth_network_error_stays_in_codex_error_path(monkeypatch) -> None:
    def fake_token():
...
@pytest.mark.asyncio
async def test_codex_provider_uses_proxy_for_oauth_and_response_request(monkeypatch) -> None:
    proxy = "http://127.0.0.1:23458"
    original_env = {
        name: f"http://original-{name.lower()}.example:8080"
    ...
    def fake_token():
        seen["oauth_env"] = {
            name: os.environ.get(name) for name in _CODEX_OAUTH_PROXY_ENV_VARS
        }
    ...
    assert seen["oauth_env"] == {name: proxy for name in _CODEX_OAUTH_PROXY_ENV_VARS}
